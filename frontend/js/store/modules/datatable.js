...
const actions = {
  ...
  [ACTIONS.BULK_EXPORT_PDF] ({ commit, state, dispatch }) {
    api.bulkExportPDF(state.bulk.join(), function (resp) {
      commit(NOTIFICATION.SET_NOTIF, { message: resp.data.message, variant: resp.data.variant })
      dispatch(ACTIONS.GET_DATATABLE)
    })
  },
  [ACTIONS.BULK_EXPORT_XLS] ({ commit, state, dispatch }) {
    api.bulkExportXLS(state.bulk.join(), function (resp) {
      commit(NOTIFICATION.SET_NOTIF, { message: resp.data.message, variant: resp.data.variant })
      dispatch(ACTIONS.GET_DATATABLE)
    })
  }
